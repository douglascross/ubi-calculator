  ...
  constructor(
    ...
    this.ws.segments.subscribe(segments => {
      ...
      for (var i = 0; i < 80; i += 10) {
        ...
        const y1 = Math.round(this.ws.getY(x1));
        const y2 = Math.round(this.ws.getY(x2));
        this.taxSegments.push({
      ...
      // for (var i = 99.9; i <= 100; i += 0.01) {
      //   const x = households * (i + 0.005) / 100;
      //   const y = this.ws.getY(x);
      //   const x1 = Math.round(households * i / 100);
      //   const x2 = Math.round(households * (i + 0.01) / 100);
      //   const y1 = Math.round(this.ws.getY(x1));
      //   const y2 = Math.round(this.ws.getY(x2));
      //   this.taxSegments.push({
      //     name: 'Top ' + (100 - i).toPrecision(1) + "%",
      ...
      const total = this.totalWealth = this.taxSegments.map(x => x.total).reduce((o, x) => o + x);
      this.taxSegments.forEach(x => {
